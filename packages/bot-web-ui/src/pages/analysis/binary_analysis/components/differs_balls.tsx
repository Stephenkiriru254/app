...
const DiffersBalls = ({
    ...
    const findMinMaxKeys = (input: Record<string, number>): { maxKey: string; minKey: string } => {
        let maxKey: string = '';
        let minKey: string = '';
        let secondMinKey: string = '';
        let maxValue: number = -Infinity;
        let minValue: number = Infinity;
        let secondMinValue: number = Infinity;
        // Use querySelectorAll with type assertion to HTMLElement because querySelectorAll returns NodeList of Elements
        const progressBalls = document.querySelectorAll<HTMLElement>('.progress');
        ...
        for (const [key, value] of Object.entries(input)) {
            ...
            if (value < minValue && value > 0) {
                ...
                minValue = value;
                minKey = key;
            } else if (value < secondMinValue && value > 0 && value !== minValue) {
        ...
        progressBalls.forEach(ball => {
            ...
            if (parseInt(ball.dataset.number!) === parseFloat(secondMinKey)) {
    ...
    return (
        <div>
            <div className='differs_container'>
                ...
                <div className='progress second' data-number='6' onClick={() => buy_contract('6')}>
                    <h3>6</h3>
                    <h4>
